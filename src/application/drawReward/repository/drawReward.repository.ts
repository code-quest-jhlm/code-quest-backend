...
export class DrawRewardRepository {
  ...
  async getAllRewardsByDrawId(idDraw: string) {
    const queryBuilder = this.drawRewardRepository.createQueryBuilder('draw_reward')
    queryBuilder.leftJoinAndSelect('draw_reward.id_reward', 'rewards')
    queryBuilder.where('draw_reward.id_draw = :idDraw', { idDraw })
    return (await queryBuilder.getMany()).map(
      (drawReward) => drawReward.id_reward
    )
  }
}
